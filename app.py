...
<style>
    ...
    /* Expander Cards for Recommendations */
...
if not uploaded_file:
    # Beautiful landing prompt when no file
    st.markdown("""
        <div class="hero-banner">
            <div class="hero-title">🌟 AI-Powered Dental Network Planner</div>
            <div class="hero-subtitle">Intelligent demographic targeting & clinic expansion strategy.</div>
        </div>
    """, unsafe_allow_html=True)
    st.info("👈 Please upload your CDC PLACES dataset (CSV) in the sidebar to run the models and view insights.", icon="📂")
    st.stop()
...
api_key = st.sidebar.text_input("Claude API Key", type="password", help="Paste your Anthropic API key to activate the Chatbot.")
...
with tab_overview:
    ...
    # 4 Top KPIs in a nice row
    c1, c2, c3, c4 = st.columns(4)
    c1.metric("Total Markets", f"{len(df_final):,}")
    avg_prev = df_final['Prevalence'].mean()
    c2.metric("Mean Target Prevalence", f"{avg_prev:.1f}%")
    ...
    with col_map:
        ...
        # Ensure we have FIPS or State to plot
        if 'FIPS' in df_final.columns:
            # Aggregate FIPS so we only have one value per county boundary
            df_map = df_final.groupby('FIPS', as_index=False).agg({'Demand_Index': 'mean', 'Prevalence': 'mean', 'County': 'first'})
            fig_map = px.choropleth(df_map, geojson="https://raw.githubusercontent.com/plotly/datasets/master/geojson-counties-fips.json", locations='FIPS', color='Demand_Index',
                                   color_continuous_scale="Plasma",
                                   scope="usa",
                                   hover_name="County",
                                   hover_data=["Prevalence", "Demand_Index"],
            ...
            # Aggregate to State level if we don't have county shape
            df_state = df_final.groupby('State')['Demand_Index'].mean().reset_index()
            fig_map = px.choropleth(df_state, locations='State', locationmode="USA-states", color='Demand_Index', scope="usa", color_continuous_scale="Plasma", template="plotly_dark")
            fig_map.update_geos(bgcolor="rgba(0,0,0,0)", showcoastlines=True, coastlinecolor="rgba(255,255,255,0.2)", showland=True, landcolor="rgba(30,41,59,0.5)")
            fig_map.update_layout(margin={"r":0,"t":0,"l":0,"b":0}, paper_bgcolor="rgba(0,0,0,0)", plot_bgcolor="rgba(0,0,0,0)")
            st.plotly_chart(fig_map, use_container_width=True)
        else:
            fig_scatter = px.scatter(df_final, x="Prevalence", y="Demand_Index", size="Demand_Index", color="Demand_Index", hover_name="County", template="plotly_dark")
    ...
    with col_bar:
        ...
        fig_bar = px.bar(df_top.head(10), x="Demand_Index", y="County", orientation='h', color='Prevalence', color_continuous_scale='Purpor', template="plotly_dark")
        fig_bar.update_layout(yaxis={'categoryorder':'total ascending'}, margin={"r":0,"t":30,"l":0,"b":0}, paper_bgcolor="rgba(0,0,0,0)", plot_bgcolor="rgba(0,0,0,0)")
    ...
    if not raw_vs_ai:
        ...
        with col_c1:
            ...
            if x_col in df_final.columns and y_col in df_final.columns:
                ...
                st.info("Insufficient features for 2D clustering plot.")
            st.markdown('</div>', unsafe_allow_html=True)
...
with tab_recs:
    ...
    # Data viewer
    st.divider()
    with st.expander("Explore Full Analyzed Dataset (CSV)"):
...
with tab_chat:
    ...
    if not api_key:
        ...
        if "chat_history" not in st.session_state:
            st.session_state.chat_history = [{"role": "assistant", "content": "Hello! I am ready to analyze your currently loaded dental demand metrics. What would you like to know?"}]
